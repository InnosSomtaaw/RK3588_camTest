...
RKNN_INFERENCER::RKNN_INFERENCER(QObject *parent)
{
    hasInited=false;
...
int RKNN_INFERENCER::fullTest(int argc, char **argv)
{
      memset(&src_rect, 0, sizeof(src_rect));
      memset(&dst_rect, 0, sizeof(dst_rect));
      memset(&src, 0, sizeof(src));
...
//    memset(input_attrsDyn, io_num.n_input, sizeof(rknn_tensor_attr));
    for (int i = 0; i < io_num.n_input; i++) {
...
//    memset(output_attrsDyn, io_num.n_output, sizeof(rknn_tensor_attr));
    for (int i = 0; i < io_num.n_output; i++) {
...
int RKNN_INFERENCER::RunOnce()
{
    if (img_input1.empty() || !ipcMutex.tryLock())
        return -1;

    cvtColor(img_input1, img_output1, cv::COLOR_BGR2RGB);
