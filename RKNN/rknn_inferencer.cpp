...
double RKNN_INFERENCER::__get_us(struct timeval t) { return (t.tv_sec * 1000000 + t.tv_usec); }
...
unsigned char* RKNN_INFERENCER::read_file_data(const char* filename, int* model_size)
{
  FILE* fp;
  unsigned char* data;
...
int RKNN_INFERENCER::write_data_to_file(const char *path, char *data, unsigned int size) {
  FILE *fp;
...
int RKNN_INFERENCER::init_model(const char* model_path, rknn_app_context_t* app_ctx) {
  int ret;
  rknn_context ctx;
...
int RKNN_INFERENCER::release_model(rknn_app_context_t* app_ctx) {
...
int RKNN_INFERENCER::inference_model(rknn_app_context_t* app_ctx, image_frame_t* img, detect_result_group_t* detect_result) {
  int ret;
...
//  img.format = RK_FORMAT_RGB_888;
  ...
  ret = RKNN_INFERENCER::inference_model(ctx, &img, &detect_result);
  if (ret != 0) {
...
int RKNN_INFERENCER::process_video_file(rknn_app_context_t* ctx, const char* path)
{
  int video_size;
...
int RKNN_INFERENCER::full_test(int argc, char** argv)
{
  int status = 0;
  int ret;
...
int RKNN_INFERENCER::process_video_rtsp(rknn_app_context_t* ctx, const char* url)
{
  mk_config config;
